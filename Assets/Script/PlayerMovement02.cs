...
public class PlayerMovement02 : MonoBehaviour
{
    ...
    [Header("摇杆控制")]
    [SerializeField] private Joystick variableJoystick; // 虚拟摇杆引用
    [SerializeField] private float joystickDeadZone = 0.2f; // 摇杆死区，小于此值不响应
    ...
    void HandleJoystickInput()
    {
        ...
        {
            if (horizontalInput < 0 && currentLane > 0) // 向左移动
            {
                currentLane--;
            }
            else if (horizontalInput > 0 && currentLane < 2) // 向右移动
            {
                currentLane++;
            }
        }
        ...
        // 跳跃（上滑）
        if (verticalInput > joystickDeadZone && IsGrounded() && !isSliding)
        {
            Jump();
        }
